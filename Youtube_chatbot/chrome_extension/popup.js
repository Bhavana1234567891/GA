/**
 * Popup chat UI that talks to the FastAPI backend.
 */
...
async function init() {
  ...
  chrome.runtime.sendMessage({ type: "GET_VIDEO_ID" }, async (response) => {
    if (!response || !response.videoId) {
      setStatus("error", "No YouTube video detected");
      if (welcomeEl) welcomeEl.textContent = "Please open a YouTube video first, then click this extension.";
      return;
    }
    ...
    try {
      ...
      if (welcomeEl) welcomeEl.textContent = `Video loaded! Ask me anything about it.`;
    } catch (err) {
      setStatus("error", "Server offline?");
      if (welcomeEl) welcomeEl.textContent = `Error: ${err.message}. Make sure the backend server is running.`;
    }
  });
}
...
function addBotMessage(data) {
  ...
  badge.textContent = data.is_grounded ? "✓ Grounded" : "⚠ Check sources";
  ...
  if (data.sources && data.sources.length > 0) {
    ...
    data.sources.forEach((s, i) => {
      ...
      item.appendChild(document.createTextNode(` ${s.text.substring(0, 100)}...`));
...
function makeTimestampsClickable(text) {
  return text.replace(
    ...
    (match, start) => {
      ...
      return `<span class="timestamp-link" data-seek="${seconds}">${match}</span>`;
    }
  );
}
...
document.addEventListener("click", (e) => {
  if (e.target.classList.contains("timestamp-link") && e.target.dataset.seek) {
